...
# Dataset of Violent scores 
violent <- read.csv('https://raw.githubusercontent.com/adidust4/COMPAS-112-Final-Project/main/cox-violent-parsed.csv')

# Dataset grouping age and scores
violent1 <- violent %>%
  filter(score_text!= "N/A") %>% 
  select(race, age_cat, score_text)
...
ui <- fluidPage(
  ...
  fluidRow( 
    column(6, offset = 3,
           ...
           h4("This plot shows the distribution of scores by age category and race."),
           plotOutput("AgeVisualization"),
           p('As seen in the plot above, older individuals were less likely to recieve a medium or high risk score than younger individuals. Of course this varies some by race. For example, Native Americans were more likely to be predicted as high risk for older individuals than younger individuals. Generally though, the algorithm seems to predict old age as a low risk factor.'),
...
server <- function(input, output) {
  ...
  # visualization comparing age, race, and score
  output$AgeVisualization <-  renderPlot({
    ggplot(violent1, aes(x= factor(age_cat, levels=c('Greater than 45','25 - 45','Less than 25')), fill=factor(score_text, level=c('High', 'Medium','Low')))) +
      geom_bar(position = "fill") +
      facet_wrap(~race) +
      scale_fill_manual(values = c(red, yellow, green)) +
      scale_x_discrete(labels = c('45+', '25-45', "<25")) +
      labs(title = "", x = "Age Category", y = "Proportion with Given Score", fill = "Risk Score") + 
      our_theme()
  })
