...
```{r}
violent1 <- violent %>% select(race, sex, age_cat, score_text) %>% group_by(race,sex,age_cat,score_text) %>% summarise(count=n()) %>% na.omit()
```
```{r}
ggplot(violent1, aes(x=score_text,y=count,fill=race)) +geom_bar(stat="identity") + facet_wrap("age_cat") + xlab("risk level of recidivism")
```
