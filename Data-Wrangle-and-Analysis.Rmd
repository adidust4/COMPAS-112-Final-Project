...
sex_race_score_recidivism <- scores %>%
  group_by(Sex_Code_Text, Ethnic_Code_Text) %>%
  summarize(avg_score = mean(DecileScore))
...
maritalstatus_race_sex_recidivism <- scores %>% 
  group_by(Ethnic_Code_Text, MaritalStatus) %>% 
  summarize(avg_score = mean(DecileScore))

ggplot(maritalstatus_race_sex_recidivism, aes(x = reorder(Ethnic_Code_Text, -avg_score), y = avg_score, fill = MaritalStatus)) +
  geom_bar(stat = 'identity', position = 'identity', alpha = 0.5) +
  labs(title = "Risk of Recidivism Score", x = "Race", y = "Average Score", color = "Marital Status") +
  theme_minimal() + 
  scale_fill_grey()
```
